...
function menuFunction() {
  ...
  if (menu.className === "menu" && header.className === "header") {
    //if the 'menu' class is 'menu' and 'header' class is 'header' (we want to change them simultaneously)
...
function searchFunction() {
  ...
  for (i = 0; i < article.length; i++) {
    //For loop the article variable (go through the index of the elements in the array)
    h2 = article[i].getElementsByTagName("h2")[0]; //h2 = the relevant 'i' with the h2 tag
    if (h2.innerHTML.toUpperCase().indexOf(filter) > -1) {
      //if the h2 tag (in uppercase) has an index (has the search term). -1 means that it doesn't
